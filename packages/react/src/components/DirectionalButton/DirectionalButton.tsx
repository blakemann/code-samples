...
export default function DirectionalButton({ direction = Direction.Up, onPressed, onReleased }:Props) {
  ...
  const gradients = useRef<GradientData>({
    edge: {
  ...
  function onMouseDown():void {
    if (shimmer.current) {
      ...
      shimmerTimeline.current.to(shimmer.current, { opacity: 0.4, duration: 0.1, ease: 'power1.easeIn' });
      shimmerTimeline.current.to(shimmer.current, { opacity: 0, duration: 0.5, ease: 'power1.easeOut', delay: 0.1 });
      shimmerTimeline.current.to(size, { y1: -540, y2: 60, duration: 0.5, ease: 'power1.easeIn', onUpdate }, 0);
    }
  }

  // template

  const faceStops:ReactNode[] = getGradientStopsTemplate(gradients.current.face.stops[direction]);
  const edgeStops:ReactNode[] = getGradientStopsTemplate(gradients.current.edge.stops[direction]);
  const shimmerStops:ReactNode[] = getGradientStopsTemplate(gradients.current.shimmer.stops);
      ...
      <span className={styles['rotation-wrapper']}>
        ...
        >
          ...
          >
            <defs>
              <linearGradient
                id={gradients.current.face.id}
                {...gradients.current.face.sizes[orientation]}
                gradientUnits="userSpaceOnUse"
              >
                {faceStops}
              </linearGradient>
              <linearGradient
                id={gradients.current.edge.id}
                {...gradients.current.edge.sizes[orientation]}
                gradientUnits="userSpaceOnUse"
              >
                {edgeStops}
              </linearGradient>
              <linearGradient
                id={gradients.current.shimmer.id}
                ref={shimmerGradient}
                {...gradients.current.shimmer.size}
                gradientUnits="userSpaceOnUse"
              >
                {shimmerStops}
              </linearGradient>
            ...
            <path
              ...
              fill={`url(#${gradients.current.shimmer.id})`}
              d="M49.93,15.57c-.24,4.93-.69,17.03-.92,23.3,0,0,.52,4.88-3.08,8.63-3.16,3.29-8.4,9.46-12.35,13.33-3.65,3.58-4.95,4.76-8.51,4.91-3.56-.15-4.85-1.33-8.51-4.91-3.94-3.86-9.19-10.04-12.35-13.33-3.6-3.75-3.08-8.63-3.08-8.63-.23-6.27-.68-18.37-.92-23.3C-.02,10.65-3.1.04,25.07,0c28.16.04,25.09,10.65,24.86,15.57Z"
            />
            <path
              className={styles['button-highlight']}
              fill={`url(#${gradients.current.face.id})`}
              stroke={`url(#${gradients.current.edge.id})`}
